...
const TestimonialSlider = ( { subtitle, title, items } ) => {
  return (
    <>
      {/* reviews */}
      <div className="mil-content-pad mil-p-100-100" style={{"backgroundImage": "url(/img/shapes/5.png)", "backgroundSize": "100%"}}>
                      ...
                      >
                        {items.map((item, key) => (
                            <SwiperSlide className="swiper-slide" key={`testimonial-slider-item-${key}`}>
                                <div className={`mil-review-frame mil-review-${key} mil-text-center`} data-swiper-parallax="-250" data-swiper-parallax-opacity="0" data-image={getDirectusImageURL(item.image)} data-name={item.name}>
                                    <div className="mil-fade-up">
                                        <h3 className="mil-mb-10 mb-3">{item.name}</h3>
                                    </div>
                                    <p className="mil-fade-up">{item.text}</p>
                                </div>
                            </SwiperSlide>
                        ))}
                      </Swiper>

                      <div className="mil-slider-nav mil-reviews-nav mil-fade-up">
